...
def select_good_veto_muons(
    events: ak.Array, muon_pt_threshold_min: int, muon_pt_threshold_max: int, abs_muon_eta: float, muon_id_wp: str, muon_iso_wp: str
) -> ak.highlevel.Array:
    """
    ...
    """
    # muon pT threshold
    muon_pt_mask = (events.Muon.pt > muon_pt_threshold_min) & (events.Muon.pt < muon_pt_threshold_max)

    # electron pseudorapidity mask
    muon_eta_mask = np.abs(events.Muon.eta) < abs_muon_eta
